$(document).ready(function(){
	var funcion = '';
	var id_usuario = $('#id_usuario').val();
	var edit=false;
//console.log(id_usuario);
buscar_usuario(id_usuario);
function buscar_usuario(dato){
	funcion = 'buscar_usuario';
	$.post('../controlador/UserController.php',{dato, funcion},(response)=>{
		let nombre ='';
		let apellidos ='';
		let edad ='';
		let cedula ='';
		let tipo ='';
		let telefono ='';
		let residencia ='';
		let correo ='';
		let sexo ='';
		let adicional ='';
		const usuario = JSON.parse(response);
		/*agregado de templates*/
		nombre+=`${usuario.nombre}`;
		apellidos+=`${usuario.apellidos}`;
		edad+=`${usuario.edad}`;
		cedula+=`${usuario.cedula}`;
		tipo+=`${usuario.tipo}`;
		telefono+=`${usuario.telefono}`;
		residencia+=`${usuario.residencia}`;
		correo+=`${usuario.correo}`;
		sexo+=`${usuario.sexo}`;
		adicional+=`${usuario.adicional}`;
		$('#nombre_usuario').html(nombre);
		$('#apellido_usuario').html(apellidos);
		$('#edad').html(edad);
		$('#cedula').html(cedula);
		$('#usuario_tipo').html(tipo);
		$('#telefono_usuario').html(telefono);
		$('#residencia_usuario').html(residencia);
		$('#correo_usuario').html(correo);
		$('#sexo_usuario').html(sexo);
		$('#adicional_usuario').html(adicional);
		$('#img_perfil1').attr('src',usuario.avatar);
		$('#img_perfil2').attr('src',usuario.avatar);
		$('#img_perfil3').attr('src',usuario.avatar);
		$('#img_perfil4').attr('src',usuario.avatar);
		})
	}
	$(document).on('click', '.edit',(e)=>{
		funcion='capturar_datos';
		edit=true;
		$.post('../controlador/UserController.php',{funcion,id_usuario},(response)=>{
			usuario = JSON.parse(response);
			$('#telefono').val(usuario.telefono);
			$('#residencia').val(usuario.residencia);
			$('#correo').val(usuario.correo);
			$('#sexo').val(usuario.sexo);
			$('#adicional').val(usuario.adicional);
		})
	});
	$('#form-usuario').submit(e=>{
		if (edit==true){
			let telefono=$('#telefono').val();
			let residencia=$('#residencia').val();
			let correo=$('#correo').val();
			let sexo=$('#sexo').val();
			let adicional=$('#adicional').val();
			funcion='editar_usuario';
			$.post('../controlador/UserController.php',{id_usuario,funcion,telefono,residencia,correo,sexo,adicional},(response)=>{
				if (response=='editado') {
					$('#editado').hide('slow');
					$('#editado').show(1500);
					$('#editado').hide(2000);
					$('#form-usuario').trigger('reset');
				}
				edit=false;
				buscar_usuario(id_usuario);
			})
		}
		else{
			$('#noeditado').hide('slow');
			$('#noeditado').show(1500);
			$('#noeditado').hide(2000);
			$('#form-usuario').trigger('reset');
		}
		e.preventDefault();
	});

	$('#form-pass').submit(e=>{
		let oldpass=$('#oldpass').val();
		let newpass=$('#newpass').val();
		funcion='cambiar_pass';
		$.post('../controlador/UserController.php',{id_usuario,funcion,oldpass,newpass},(response)=>{
			if (response=='update') {
				$('#update').hide('slow');
				$('#update').show(1500);
				$('#update').hide(2000);
				$('#form-pass').trigger('reset');
			}
			else{
				$('#noupdate').hide('slow');
				$('#noupdate').show(1500);
				$('#noupdate').hide(2000);
				$('#form-pass').trigger('reset');
			}
		})
		e.preventDefault();
	})
	$('#form-photo').submit(e=>{
		let formData = new FormData($('#form-photo')[0]);
		$.ajax({
			url:'../controlador/UserController.php',
			type:'POST',
			data:formData,
			cache:false,
			processData:false,
			contentType:false
		}).done(function(response){
			const json =JSON.parse(response);
			if (json.alert=='edit') {
				$('#img_perfil1').attr('src',json.ruta);
				$('#edit').hide('slow');
				$('#edit').show(1500);
				$('#edit').hide(2000);
				$('#form-photo').trigger('reset');
			}
			else{
				$('#noedit').hide('slow');
				$('#noedit').show(1500);
				$('#noedit').hide(2000);
				$('#form-photo').trigger('reset');
			}
			buscar_usuario(id_usuario);
		});
		e.preventDefault();
	})
})